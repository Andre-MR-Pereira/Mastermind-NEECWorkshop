...
class Board:
    def __init__(self):				# happens when a board is created
        ...
        self.board = ["  ".join(["><" for c in range(0, self.numColors)]) for i in range(
            0, self.numMaxColors)]  # starts board guess array with attempts as ><
    ...
    def printBoard(self, r={"right": -1, "placed": -1}):
        ...
        self.printElement('', f='+')
        self.printElement(self.getColorStr(
            [str(x) for x in range(1, 9)], 18, elem='n', sep=''), c=' ', f='|')
        self.printElement('', f='+')
        for i in self.board:
    ...
    def getColorStr(self, sequence, l=16, elem='  ', sep='  ', right=' ', placed=' '):
        ...
        sep = sep.join(
            [f.format('1;48', x, ' ' + (str(x) if elem == 'n' else ' ')) for x in sequence])

        rhs = ((self.center-l)//2)+(0 if self.center %
                                    2 == 0 else 1)  # right formating spacing
    ...
    def updateBoard(self, i, t, r):
        self.board[i] = self.getColorStr(t, 16, right=str(
            r.get("right")), placed=str(r.get("placed")))
    ...
    def Guess(self, attempt, iteration):
        # dictionary with {right:'',placed:''}
        answer = match(attempt, self.key)
